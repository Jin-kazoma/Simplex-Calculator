...
function solveSimplex(){
    ...
    while(true)
        ...
        // PIVOT ELIMINATION - styled output
let pivot = T[pivotRow][pivotCol];
let pivotHTML = `<div class="pivot-elimination">`;

pivotHTML += `<span class="subheading">Step 4: Pivot Elimination</span>`;
pivotHTML += `<b>Pivot Element (k):</b> ${toFraction(pivot)} 
              &nbsp;&nbsp; <b>1 / k:</b> ${toFraction(1/pivot)}<br><br>`;

// Normalize pivot row
pivotHTML += `<span class="subheading">Normalize Pivot Row (÷ ${toFraction(pivot)})</span>`;
for(let j=0; j<cols; j++){
    let original = T[pivotRow][j];
    let newVal = original / pivot;
    pivotHTML += `<div class="calc">
        ${toFraction(original)} ÷ ${toFraction(pivot)} = ${toFraction(newVal)}
    </div>`;
}

// Perform pivot row division
let p = pivot;
for(let j=0; j<cols; j++) {
    T[pivotRow][j] /= p;
}
...
pivotHTML += `<span class="subheading">Row Elimination</span>`;

for(let i=0;i<rows;i++){
    if(i !== pivotRow){
        let f = T[i][pivotCol];
        if(Math.abs(f) > 1e-10){
            let rowLabel = (i === rows - 1)
                ? "Z − x(pivot)"
                : `s${basic[i] - n + 1} − x(pivot)`;

            pivotHTML += `<div class="subheading">${rowLabel}</div>`;

            for(let j=0; j<cols; j++){
                let original = T[i][j];
                let subtractVal = f * T[pivotRow][j];
                let newVal = original - subtractVal;

                pivotHTML += `<div class="calc">
                    ${toFraction(original)} − 
                    (${toFraction(f)} × ${toFraction(T[pivotRow][j])}) 
                    = ${toFraction(newVal)}
                </div>`;
            }
        }
...
pivotHTML += `</div>`;
resultArea.innerHTML += pivotHTML;
